...
down_revision: str | None = "bd634a654a8a"
...
def upgrade() -> None:
    ...
    op.add_column(
        "github_repos",
        sa.Column(
            "applicable_scenarios", sa.JSON(), nullable=False, server_default=sa.text("'[]'")
        ),
    )
    op.add_column(
        "github_repos",
        sa.Column(
            "adoption_suggestions", sa.JSON(), nullable=False, server_default=sa.text("'[]'")
        ),
    )
    op.add_column("github_repos", sa.Column("testing_value_score", sa.Integer(), nullable=True))
    op.add_column(
        "github_repos",
        sa.Column("intel_status", sa.String(length=30), nullable=False, server_default="none"),
    )
    op.create_index(
        op.f("ix_github_repos_testing_value_score"),
        "github_repos",
        ["testing_value_score"],
        unique=False,
    )
    op.create_index(
        op.f("ix_github_repos_intel_status"), "github_repos", ["intel_status"], unique=False
    )
