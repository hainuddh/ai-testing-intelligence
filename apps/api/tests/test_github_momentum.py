...
def test_score_components_are_deterministic():
    ...
    m = momentum_score(
        stars=1000, star_delta_24h=30, star_delta_7d=150, pushed_at=pushed, now=now
    )
    assert m.score == 65.0
...
def test_activity_degrades_with_age():
    ...
    m_recent = momentum_score(stars=30000, star_delta_24h=0, star_delta_7d=0, pushed_at=recent, now=now)
...
def test_score_caps_at_100():
    m = momentum_score(stars=1000, star_delta_24h=1000, star_delta_7d=1000, pushed_at=datetime.now(UTC))
