...
最终准入且测试价值达到 60 分的内容会进入独立补全队列，Worker 每轮最多处理 3 条，并从
原文实际锚点中确定性提取最多 5 条相关资料链接；该阶段不重新调用模型，也不会请求链接
指向的页面。升级前已经分析的高价值内容也会由这个队列逐步补齐。
未配置模型时，内容保持 `pending`，主雷达有意不展示未经分析的通用资讯。模型密钥不得提交
到 Git，也不要写入日志或文档。

默认只把 RSS/Atom 提供的标题和摘要发送给模型服务，不会在模型分析前额外下载并发送文章
全文。高价值内容在模型分析完成后可能读取原文页面用于本地提取相关链接，但页面正文不会因此
再次发送给模型。如果明确接受相应的数据传输、版权和模型调用成本风险，可设置
`ATI_ANALYSIS_FETCH_FULL_CONTENT=true`。
...
信源管理支持“微信公众号”和“微博”两类平台信源。首期接入遵循以下边界：

- 可以为平台信源配置来源明确、已获授权的 RSS/Atom 地址；系统沿用现有订阅采集、URL 去重和分析链路。
- 不允许为平台信源配置网页自动采集端点，避免把账号主页或公开页面当作持续爬取入口。
- 维护者和管理员可以在信源卡片中选择“录入内容”，人工填写标题、原文地址、摘要和可选发布日期。
- 人工录入不会请求微信或微博页面，也不会要求 Cookie、账号密码或抓包令牌；填写内容将直接进入待分析队列。
- 同一个原文 URL 只能录入一次。录入人员应确认内容来源和处理权限，并保留原文证据链接。

若后续需要同步自有或客户授权的公众号，应优先接入微信官方发布接口或微信第三方平台授权；若需要持续监控任意微博账号、关键词或评论，应先评估微博商业数据 API，不应通过验证码绕过、Cookie 池或页面反爬对抗实现。

### 7.4 导出 Markdown 测试情报报告
